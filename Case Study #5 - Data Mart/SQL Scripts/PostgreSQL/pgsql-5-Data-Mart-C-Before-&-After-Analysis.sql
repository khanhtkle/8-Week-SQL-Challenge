...
WITH cumulative_sales_cte AS
  (SELECT SUM(CASE
                  WHEN week_number BETWEEN DATE_PART('week', DATE '2020-06-15') - 12 AND DATE_PART('week', DATE '2020-06-15') - 1 THEN sales
              END) AS cml_sales_12wk_pre,
          SUM(CASE
                  WHEN week_number BETWEEN DATE_PART('week', DATE '2020-06-15') AND DATE_PART('week', DATE '2020-06-15') + 11 THEN sales
...
SELECT cml_sales_12wk_pre,
       ...
       (100.0 * (cml_sales_12wk_post - cml_sales_12wk_pre) / cml_sales_12wk_pre)::DECIMAL(5, 2) AS variance_pct
FROM cumulative_sales_cte;
...
SELECT calendar_year,
       ...
       cml_sales_4wk_pre_2020 - cml_sales_same_4wk_pre_period::REAL AS same_pre_period_variance,
	   (100.0 * (cml_sales_4wk_pre_2020 - cml_sales_same_4wk_pre_period) / cml_sales_same_4wk_pre_period)::DECIMAL(5, 2) AS same_pre_period_variance_pct,
	   cml_sales_same_4wk_post_period,
	   cml_sales_4wk_post_2020,
       cml_sales_4wk_post_2020 - cml_sales_same_4wk_post_period::REAL AS same_pre_period_variance,
	   (100.0 * (cml_sales_4wk_post_2020 - cml_sales_same_4wk_post_period) / cml_sales_same_4wk_post_period)::DECIMAL(5, 2) AS same_post_period_variance_pct
FROM cumulative_sales_cte, 
...
SELECT calendar_year,
       ...
       cml_sales_12wk_pre_2020 - cml_sales_same_12wk_pre_period::REAL AS same_pre_period_variance,
	   (100.0 * (cml_sales_12wk_pre_2020 - cml_sales_same_12wk_pre_period) / cml_sales_same_12wk_pre_period)::DECIMAL(5, 2) AS same_pre_period_variance_pct,
	   cml_sales_same_12wk_post_period,
	   cml_sales_12wk_post_2020,
       cml_sales_12wk_post_2020 - cml_sales_same_12wk_post_period::REAL AS same_pre_period_variance,
	   (100.0 * (cml_sales_12wk_post_2020 - cml_sales_same_12wk_post_period) / cml_sales_same_12wk_post_period)::DECIMAL(5, 2) AS same_post_period_variance_pct
FROM cumulative_sales_cte, 
