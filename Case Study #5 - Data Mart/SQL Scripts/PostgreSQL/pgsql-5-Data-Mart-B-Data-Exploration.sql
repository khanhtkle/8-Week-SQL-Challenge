...
SELECT calendar_year,
       ...
       (100.0 * SUM(CASE
                    ...
                    END) / SUM(sales))::DECIMAL(5, 2) AS retail_sales_pct,
       (100.0 * SUM(CASE
                        WHEN platform = 'Shopify' THEN sales
                    END) / SUM(sales))::DECIMAL(5, 2) AS shopify_sales_pct
FROM data_mart.clean_weekly_sales
GROUP BY 1, 2
ORDER BY 1, 2;
...
SELECT calendar_year,
       (100.0 * SUM(CASE
                    ...
                    END) / SUM(sales))::DECIMAL(5, 2) AS sales_by_families_pct,
       (100.0 * SUM(CASE
                        WHEN demographic = 'Couples' THEN sales
                    END) / SUM(sales))::DECIMAL(5, 2) AS sales_by_couples_pct,
       (100.0 * SUM(CASE
                        WHEN demographic = 'unknown' THEN sales
                    END) / SUM(sales))::DECIMAL(5, 2) AS sales_by_unknown_pct
FROM data_mart.clean_weekly_sales
GROUP BY 1
ORDER BY 1;
...
SELECT age_band,
       ...
       (ROUND(100.0 * SUM(sales) / total_retail_sales, 2))::DECIMAL(5, 2) AS retail_sales_by_age_band_and_demographic_pct
FROM data_mart.clean_weekly_sales, 
     retail_sales_cte
WHERE platform = 'Retail'
...
SELECT calendar_year,
       platform,
       (AVG(avg_transaction)::DECIMAL(5,2))::REAL AS average_of_avg_transaction,
       (ROUND(1.0 * SUM(sales) / SUM(transactions), 2))::REAL AS weighted_avg_transaction
FROM data_mart.clean_weekly_sales
GROUP BY 1, 2
ORDER BY 1, 2;
